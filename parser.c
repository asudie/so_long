...
/*   Updated: 2024/06/08 13:35:37 by asmolnya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (pos_x < 0 || pos_x >= data->map->map_height || pos_y < 0
		|| pos_y >= data->map->map_length)
		return ;
	if (map_check[pos_x][pos_y] == '0' || map_check[pos_x][pos_y] == 'P'
		|| map_check[pos_x][pos_y] == 'C' || map_check[pos_x][pos_y] == 'E')
	{
...
int	check_paths(int pos_x, int pos_y, char **map_check, t_data *data)
{
	...
	if (ft_element_count(map_check, data, 'E') != 0
		|| ft_element_count(map_check, data, 'C') != 0)
		return (0);
	return (1);
}
